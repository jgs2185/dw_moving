    ...
    void initializeSensor(ProgramArguments &arguments)
    {
        ...
        canSensor = DW_NULL_HANDLE;
        {
            ...
            if( dwSAL_createSensor(&canSensor, params, sal) != DW_SUCCESS)
            {
                ...
                // Removed get Radar properties since we are using the radar through CAN
...
protected:
    ...
    void renderFrame()
    {
        ...
        //Only rendering detections, removed check for and rendering of tracks
    ...
    {
        //removed call to gRadarProperties 
        return 20;
    }
    ...
    {
        static dwRadarDetection detection;

        if(msg.size > 0){

            detection.radius = (float32_t)(int16_t)( (msg.data[2] << 8) + msg.data[3]) / 100.0; 
            detection.azimuth = (float32_t)(int16_t)( (msg.data[6] << 8) + msg.data[7]) / 100.0 * -1;
            detection.radialVelocity = (float32_t)(int16_t)( (msg.data[4] << 8) + msg.data[5]) / 100.0;
            detection.rcs = (float32_t)msg.data[1];


            float32_t angle = (float32_t)(int16_t)( (msg.data[6] << 8) + msg.data[7]) / 100.0 * -1;
            detection.x = ((float32_t)(int16_t)( (msg.data[2] << 8) + msg.data[3]) / 100.0)*cos(angle*3.14/180.);
            detection.y = ((float32_t)(int16_t)( (msg.data[2] << 8) + msg.data[3]) / 100.0)*sin(angle*3.14/180.);


            std::cout << detection.radius << " " << detection.azimuth << " " << detection.x << " " << detection.y << std::endl;
        }
    ...
    dwRadarDetection * collectScan()
    {
        ...
        while(!renderReady)
        {
            ...
            //Everything else is either a target or a radar ACK
                ...
                dataArray[targetCount] = parseCANtarget(msg);
    ...
    void computeSpin()
    {
        ...
        dwRadarDetection *packetArray = collectScan(); 
        ...
        //Removed option to use recorded data
        ...
        for (int i = 0; i < targetCount; i++) {
            ...
            dwRadarDetection newDetection = packetArray[i];
            ...
            if (status == DW_SUCCESS) {
                //removed switch for choosing detection type
